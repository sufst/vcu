...
- **Top bar**: Controls what CANoe is currently doing. Note how this is currently on _offline_ mode which means this is replaying a log file from disk, rather than using a live CAN bus via a USB dongle.
...
3. Make sure you're in _Online mode_, enable the CAN IG (interactive generator) block on CAN-S, hit start on the logging then open the CAN-IG menu (double click and move the window wherever you want)
