 ...
 */
static ULONG torque_request_queue_mem[TORQUE_REQUEST_QUEUE_SIZE
...
void can_tx_thread_entry(ULONG thread_input)
{
    ...
    while (1)
    {
        ...
        UINT ret = tx_queue_receive(&torque_request_queue,
                                    &torque_request,
                                    TX_WAIT_FOREVER);
        ...
        if (status == PM100_OK)
        {
            trace_log_event(TRACE_TORQUE_REQUEST_EVENT,
                            (ULONG) torque_request,
                            message.timestamp,
                            0,
                            0);
        }
        ...
        {
            trace_log_event(TRACE_INVERTER_ERROR,
                            (ULONG) status,
                            message.timestamp,
                            0,
                            0);
