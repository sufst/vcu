...
UINT watchdog_thread_create(TX_BYTE_POOL* stack_pool_ptr)
{
    ...
    if (ret == TX_SUCCESS)
    {
        ret = tx_queue_create(&minor_fault_queue,
                              ...
                              sizeof(minor_fault_queue_mem)
                                  / sizeof(minor_fault_queue_mem[0]));
    }
    ...
    if (ret == TX_SUCCESS)
    {
        ret = tx_queue_create(&critical_fault_queue,
                              ...
                              sizeof(critical_fault_queue_mem)
                                  / sizeof(critical_fault_queue_mem[0]));
    }
...
void watchdog_thread_entry(ULONG thread_input)
{
    ...
    {
        const UINT status
            = tx_semaphore_get(&fault_semaphore, WATCHDOG_THREAD_WAKE_TIMEOUT);
        ...
        case TX_SUCCESS:
        {
            ...
            if (tx_queue_receive(&critical_fault_queue,
                                 (VOID*) &critical_fault,
                                 TX_NO_WAIT)
                == TX_SUCCESS)
            {
            ...
            if (tx_queue_receive(&minor_fault_queue,
                                 (VOID*) &minor_fault,
                                 TX_NO_WAIT)
                == TX_SUCCESS)
            {
